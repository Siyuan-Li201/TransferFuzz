...
if [ $RESUME -le $STEP ]; then
  ...
  if [ -z "$fuzzer" ]; then
    for binary in $(echo "$binaries"); do
      ...
      opt -load /transferfuzz/libDFUZZPASS.so -DFUZZPASS $binary.0.0.preopt.bc -targets=$TMPDIR/BBtargets.txt -outdir=$TMPDIR
    ...
    rm ./indirect.txt
    /transferfuzz/temporal-specialization/SVF/Release-build/bin/wpa -print-fp -ander -dump-callgraph $fuzzer.0.0.preopt.bc
    ...
    opt -load /transferfuzz/libDFUZZPASS.so -DFUZZPASS $fuzzer.0.0.preopt.bc -targets=$TMPDIR/BBtargets.txt -outdir=$TMPDIR
