...
cd obj-aflgo; 
CFLAGS="-DFORTIFY_SOURCE=2 -fstack-protector-all -fno-omit-frame-pointer -g -Wno-error -func_trace=/transferfuzz/scripts/fuzz_functrace/cve-2016-5318.txt  $ADDITIONAL" LDFLAGS="-ldl -lutil" ../configure --disable-shared --disable-gdb --disable-libdecnumber --disable-readline --disable-sim --disable-ld
make clean; make
cat $TMP_DIR/BBnames.txt | rev | cut -d: -f2- | rev | sort | uniq > $TMP_DIR/BBnames2.txt && mv $TMP_DIR/BBnames2.txt $TMP_DIR/BBnames.txt
cat $TMP_DIR/BBcalls.txt | sort | uniq > $TMP_DIR/BBcalls2.txt && mv $TMP_DIR/BBcalls2.txt $TMP_DIR/BBcalls.txt
python3 /transferfuzz/scripts/generate_func_trace.py /transferfuzz/scripts/fuzz_functrace/cve-2016-5318.txt $TMP_DIR/edges_id.txt $TMP_DIR/Transfer_blocks.txt $TMP_DIR/Transfer_edges.txt
cd tools; 
$AFLGO/scripts/genDistance.sh $SUBJECT $TMP_DIR thumbnail
cd ../../; 
rm -rf obj-dist;
mkdir obj-dist; 
...
$AFLGO/afl-fuzz -x /transferfuzz/scripts/fuzz_dict/cve-2016-5318.txt -R out/fuzz_record.txt -F $TMP_DIR/target_map.txt -m none -c 45m -i in -o out -d -- tools/thumbnail @@ out.tif
